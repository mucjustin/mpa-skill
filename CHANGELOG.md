...
- **模板弹性规则**：用户指定格式、请求非标准交付物或明确要求简短输出时，模板骨架降级为检查清单——结构从用户，必填维度从模板
- **备选方案覆盖检查**：案例分析模板新增检查行——供给侧工具 / 需求侧评估 / 信息与问责机制（含如实上报的激励与容错）三类至少各考虑一项，考虑后排除须说明理由
- **分层标记**：案例分析模板各节标注 [证据]/[推断]/[建议]，交付时保留标记或以等价方式区分
- **A/B 回归对比方法**（`docs/validation/a-b-regression.md`）：发布前用同题双版本隔离对照检测行为漂移（安全决策等价 + 模板聚焦偏差）；已记录 2026-08-24 v1.0.0 vs v2.0.0 对照结果

### Validation

- 行为契约无变化（仅模板与验证文档新增，无 SKILL.md 与路由改动）；契约测试新增模板弹性条款、覆盖检查、分层标记与 A/B 回归文档断言
